...
function withCORS(headers, request) {
  ...
  if (request.method === 'OPTIONS' && corsMaxAge) {
    ...
    // Modified to include X-Forward-Cookie in allowed headers
    headers['access-control-allow-headers'] = 'Content-Type, Cookie, Authorization, X-Requested-With, X-Forward-Cookie, *';
  }
...
function getHandler(options, proxy) {
  ...
  return function(req, res) {
    ...
    // NEW: Handle X-Forward-Cookie header transformation
    if (req.headers['x-forward-cookie']) {
      // Set the Cookie header from X-Forward-Cookie
      req.headers['cookie'] = req.headers['x-forward-cookie'];
      // Remove the X-Forward-Cookie header as it's no longer needed
      delete req.headers['x-forward-cookie'];
    }
